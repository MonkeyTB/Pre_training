...
from chatglm.stream import TextStreamer, TextIteratorStreamer

from chatglm.chatglm_utils import load_hf_dataset, ChatGlmDataset
from chatglm.model_args import ChatGlmArgs
...
class ChatGlmModel(GenerationMixin):
    def __init__(
            self,
            model_type,
            model_name,
        ...
        self.model = model_class.from_pretrained(
            ...
            device_map={"": int(os.environ.get("LOCAL_RANK") or self.cuda_device)},  # auto
        )
    ...
    def load_lora(self):
        if self.args.use_lora:
            if self.lora_name:
                self.model = PeftModel.from_pretrained(self.model, self.lora_name, device_map={
                    "": int(os.environ.get("LOCAL_RANK") or self.cuda_device)})
                ...
                if lora_path and os.path.exists(lora_path):
                    self.model = PeftModel.from_pretrained(self.model, self.args.output_dir, device_map={
                        "": int(os.environ.get("LOCAL_RANK") or self.cuda_device)})
    ...
    @torch.no_grad()
    def stream_predict(self, sentences, history, keep_prompt=False, max_length=None, **kwargs):
        """
        ...
        #         yield self.model.generate(**inputs, **gen_kwargs, streamer=streamer)
        ...
        for new_text in streamer:
            response += self.process_response(new_text)
            new_history = history + [(sentences, response)]
    ...
    def prepare_inputs_for_generation(
        ...
        MASK, gMASK = self.tokenizer.mask_token_id, self.tokenizer.gmask_token_id
    ...
    @torch.no_grad()
    def stream_chat(self, query: str, history: List[Tuple[str, str]] = None, max_length: int = 2048,
    ...
    def stream_generate(
        ...
        print('bos:', bos_token_id, 'eos:', eos_token_id)
